...
from plotting import (
    plot_lgbm_importance, plot_lgbm_loss_curves,
...
def preproc_data(file_path, feature_columns=None):
    ...
    try:
        ...
        strat_label = (df_data["Age_category"].values.astype(int)
                       ...
                       + (df_data["Sex"].values.astype(int) + 1) * 1000)            # gender -> thousands place; 1000 > max(age+site)=179
...
def run_grid_search(X_train, Y_train, strat_train, grid_params):                    #Performs Grid Search to find best hyperparameters
    ...
    lgbm = LGBMRegressor(                                                           # base estimator with LightGBM defaults; hyperparams overridden by grid search
        objective='regression',
        metric='rmse',                                                              # evaluation metric stored in eval history and used for scoring
        random_state=42,
        n_jobs=1,                                                                   # single-threaded per fit so GridSearchCV (n_jobs=-1) owns all parallelism
        verbose=-1,                                                                 # suppress per-round console output inside GridSearchCV
    )
    ...
    search = GridSearchCV(
        ...
        n_jobs=32,                                                                  # parallel over all param combinations
        cv=cv_ind_list,
...
def train_and_evaluate(X_train_all, Y_train_all, strat_train, ID_train, X_test, Y_test, ID_test, feature_columns, output_dir, param_dict, tag): #Run 5-fold CV on training data, then train a final CatBoost model on all training subjects and evaluate it on the held-out test set
    ...
    es_rounds = param_dict.get('early_stopping_rounds', 100)                         # rounds without val-RMSE improvement before stopping
    clean_params = {k: v for k, v in param_dict.items() if k != 'early_stopping_rounds'}  # hyperparams passed to the estimator
    ...
    best_iters = []                                                                 # best iteration (tree count) per fold; mean used for final model n_estimators
    ...
    for cv_ind, (tr_ind, val_ind) in enumerate(cv_ind_list):                        # iterate over 5 stratified folds
        ...
        lgbm = LGBMRegressor(
            ...
            random_state=42,
            n_jobs=-1,
            verbose=-1,                                                             # suppress LightGBM's internal verbosity
            **clean_params                                                          # unpack n_estimators, learning_rate, max_depth, num_leaves, min_child_samples
        )
        # eval_set passes both train and val so we can track both curves.
        # validation_0 -> train, validation_1 -> val in evals_result()
        evals_result = {}                                                           # dict populated in-place by record_evaluation callback during fit
        lgbm.fit(
            X_tr, Y_tr,
            eval_set=[(X_tr, Y_tr), (X_val, Y_val)],                                # track RMSE on training and validation set at every boosting round
            eval_names=['train', 'val'],
            callbacks=[
                lgb.early_stopping(stopping_rounds=es_rounds, verbose=False),       # stop if val RMSE does not improve for es_rounds consecutive rounds
                lgb.record_evaluation(evals_result),                                # captures full eval history per boosting round into evals_result
                lgb.log_evaluation(period=0),                                       # suppress per-round console output (period=0 = never print)
            ],
        )

        eval_histories.append(evals_result)                                             # store loss history for this fold
        best_iters.append(lgbm.best_iteration_)                                     # best_iteration_ is 1-indexed = actual tree count used

        # Evaluate
        preds = lgbm.predict(X_val)                                                 # predict ages on the held-out validation fold
        ...
        joblib.dump(lgbm, os.path.join(model_dir, f"lgbm_model_cv{cv_ind}.pkl"))    # persist this fold's trained LightGBM model to disk

        # Permutation importance per fold (on training data, consistent with XGBoost/RF/ET pipeline)
        pi = permutation_importance(lgbm, X_tr, Y_tr, n_repeats=5, random_state=123, scoring="neg_mean_squared_error")  # shuffle each feature 5 times and measure the rise in MSE
    ...
    plot_lgbm_loss_curves(eval_histories, plots_dir, tag=tag)
    ...
    lgbm_best = joblib.load(os.path.join(model_dir, f"lgbm_model_cv{best_fold}.pkl"))  # restore best fold's trained LightGBM from disk
    logger.info(f"  Computing SHAP beeswarm on best CV fold (fold {best_fold}, RMSE={rmse_arr[best_fold]:.4f}) training data ...")
    shap_vals_best = compute_shap_values(lgbm_best, X_tr_best, feature_columns)     # exact Shapley values via TreeExplainer on training subjects
    ...
    logger.info(f"n_estimators={mean_best_iter} (mean early-stopping iteration across CV folds)")
    final_params = {k: v for k, v in clean_params.items() if k != 'n_estimators'}   # exclude n_estimators; replaced by mean_best_iter
    final_model = LGBMRegressor(
        ...
        n_jobs=-1,
        verbose=-1,                                                                 # suppress LightGBM's internal verbosity
        n_estimators=mean_best_iter,                                                # fixed at mean CV best iteration; no early stopping without eval_set
        **final_params                                                              # remaining hyperparameters from CV
    )
    final_model.fit(                                                                # train on all training subjects; no eval callback needed
        X_train_all, Y_train_all,
        callbacks=[lgb.log_evaluation(period=0)],                                   # suppress per-round output during final training
    )
    ...
    plot_lgbm_importance(final_model, plots_dir, tag=tag, feature_names=feature_columns)  # feature_columns supplies meaningful names
...
def main():
    parser = argparse.ArgumentParser(description="LightGBM Brain Age: Grid Search & Train")
    parser.add_argument('--input', type=str, required=True, help="Path to input CSV file")
    parser.add_argument('--output_dir', type=str, default="./output", help="Directory to save results")

    # Hyperparameter search ranges
    # max_depth: -1 is the LightGBM native value for unlimited depth (no conversion needed)
    parser.add_argument('--n_estimators', type=int, nargs='+', default=[100], help="Values for n_estimators (number of boosting rounds)")
    parser.add_argument('--learning_rate', type=float, nargs='+', default=[0.1], help="Values for learning_rate (step size shrinkage)")
    parser.add_argument('--max_depth', type=int, nargs='+', default=[-1], help="Values for max_depth (-1 = unlimited, LightGBM native)")
    parser.add_argument('--num_leaves', type=int, nargs='+', default=[31], help="Values for num_leaves (primary complexity control for leaf-wise growth)")
    parser.add_argument('--min_child_samples', type=int, nargs='+', default=[20], help="Values for min_child_samples (minimum samples in a leaf node)")
    ...
    DEFAULT_PARAMS = {                                                              #Run 1 - Default LightGBM hyperparameters
        'n_estimators': 100,
        'learning_rate': 0.1,
        'max_depth': -1,
        'num_leaves': 31,
    ...
    grid_params = {                                                                 # Grid Search over candidate hyperparameter ranges
        ...
        'max_depth': args.max_depth, # default [-1]
        'num_leaves': args.num_leaves, # default [31]
        'min_child_samples': args.min_child_samples, # default [20]
    }
    n_combinations = (len(args.n_estimators) * len(args.learning_rate) * len(args.max_depth) * len(args.num_leaves) * len(args.min_child_samples))                                  # total number of hyperparameter combinations to evaluate
    logger.info("")

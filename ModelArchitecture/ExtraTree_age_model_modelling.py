...
def compute_oob_curve(X_train, Y_train, param_dict, checkpoints):                   #Compute OOB RMSE at each tree-count checkpoint using warm_start
    ...
    for n in sorted(checkpoints):                                                   # iterate over checkpoints in ascending order
        et.set_params(n_estimators=n)                                               # grow forest to the next checkpoint size
        et.fit(X_train, Y_train)                                                    # fit or extend the forest (warm_start reuses existing trees)
        oob_rmse = np.sqrt(1.0 - et.oob_score_) * Y_train.std()                     # convert OOB R^2 score to approximate RMSE in years
...
def run_grid_search(X_train, Y_train, strat_train, grid_params):                    #Performs Grid Search to find best hyperparameters
    ...
    et = ExtraTreesRegressor(random_state=42, n_jobs=1)                             # single-threaded per fit so GridSearchCV owns all parallelism
...
def train_and_evaluate(X_train_all, Y_train_all, strat_train, ID_train, X_test, Y_test, ID_test, feature_columns, output_dir, param_dict, tag): #Run 5-fold CV on training data, then train a final CatBoost model on all training subjects and evaluate it on the held-out test set
    ...
    for cv_ind, (tr_ind, val_ind) in enumerate(cv_ind_list):                        # iterate over 5 stratified folds
        ...
        et.fit(X_tr, Y_tr)                                                          # train forest on this fold's training subset

        # Evaluate
        preds = et.predict(X_val)                                                   # predict ages on the held-out validation fold
        ...
        joblib.dump(et, os.path.join(model_dir, f"et_model_cv{cv_ind}.pkl"))        # persist this fold's trained ET to disk as a pickle file

        # Permutation importance per fold (on training data, consistent with XGBoost pipeline)
        pi = permutation_importance(et, X_tr, Y_tr, n_repeats=5, random_state=123, scoring="neg_mean_squared_error")  # shuffle each feature 5 times and measure the rise in MSE
    ...
    # OOB curve on full training data
    # curve is skipped for the default run and only produced when the grid search selects bootstrap=True as the best configuration.
    if param_dict.get('bootstrap', False):                                          # ET default is bootstrap=False; only compute OOB when explicitly enabled
    ...
    X_tr_best = X_train_all[tr_ind_best, :]                                         # full training subset for the best fold (~1464 subjects)
    n_shap = min(500, len(X_tr_best))                                               # cap at 500 to reduce SHAP cost otherwise this can take hours
    X_tr_shap = X_tr_best[np.random.default_rng(42).choice(len(X_tr_best), size=n_shap, replace=False)]  # random subsample; sufficient for beeswarm visualization
    et_best = joblib.load(os.path.join(model_dir, f"et_model_cv{best_fold}.pkl"))   # restore best fold's trained ET from disk
    logger.info(f"  Computing SHAP beeswarm on best CV fold (fold {best_fold}, RMSE={rmse_arr[best_fold]:.4f}) training data ({n_shap}/{len(X_tr_best)} subjects) ...")
    shap_vals_best = compute_shap_values(et_best, X_tr_shap, feature_columns)       # exact Shapley values via TreeExplainer on subsampled training subjects
    ...
    plot_tree_importance(final_model, feature_columns, plots_dir, tag=tag, model_name="Extra Trees")  # impurity-based importance via plotting.py
...
def main():
    parser = argparse.ArgumentParser(description="Extra Trees Brain Age: Grid Search & Train")
    ...
    parser.add_argument('--max_features', type=str, nargs='+', default=['1.0'], help="Values for max_features in grid search ('1.0' for all features)")
    parser.add_argument('--bootstrap', type=str, nargs='+', default=['False'], help="Values for bootstrap in grid search (True or False)")
    ...
    DEFAULT_PARAMS = {                                                              #Run 1 - Default Extra Trees hyperparameters
        'n_estimators': 100,
        'max_depth': None,
        'min_samples_split': 2,
        'max_features': 1.0,
    ...
    grid_params = {                                                                 #Grid Search over candidate hyperparameter ranges
        ...
        'max_features': max_features_vals, # default [1.0]
        'bootstrap': bootstrap_vals, # default [False]

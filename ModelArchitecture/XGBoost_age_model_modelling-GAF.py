...
from plotting import (
    plot_xgb_importance, plot_loss_curves,
...
def run_grid_search(X_train, Y_train, strat_train, grid_params):                    #Performs Grid Search to find best hyperparameters
    ...
    bst = XGBRegressor(objective='reg:squarederror', seed=42, nthread=1)            # single-threaded per fit so GridSearchCV (n_jobs=-1) owns all parallelism
    ...
    search = GridSearchCV(
        ...
        n_jobs=32,                                                                  # limit parallel workers to avoid memory spikes at fork time
        cv=cv_ind_list,
...
def train_and_evaluate(X_train_all, Y_train_all, strat_train, ID_train, X_test, Y_test, ID_test, feature_columns, output_dir, param_dict, tag): #Run 5-fold CV on training data, then train a final CatBoost model on all training subjects and evaluate it on the held-out test set
    ...
    # Directories
    model_dir = os.path.join(output_dir, tag, "models")                             # sub-folder for per-fold and final model JSON files
    ...
    skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)                # stratified 5-fold on 1800 training subjects
    ...
    eval_histories = []                                                            # store evals_result() per fold for loss-curve plot
    ...
    for cv_ind, (tr_ind, val_ind) in enumerate(cv_ind_list):                        # iterate over 5 stratified folds
        X_tr, Y_tr = X_train_all[tr_ind, :], Y_train_all[tr_ind]
        X_val, Y_val = X_train_all[val_ind, :], Y_train_all[val_ind]
        ...
        bst.fit(X_tr, Y_tr,
                ...
                verbose=False)                                                      # suppress per-round console output
        bst.get_booster().feature_names = list(feature_columns)                     # attach column names so plots use names not f0/f1/...

        eval_histories.append(bst.evals_result())                                       # store loss history for this fold
        best_iters.append(bst.best_iteration + 1)                                   # best_iteration is 0-indexed; +1 = actual tree count used

        # Evaluate
        preds = bst.predict(X_val)                                                  # predict ages on the held-out validation fold
        ...
        # Save
        bst.save_model(os.path.join(model_dir, f"best_model_cv{cv_ind}.json"))      # persist this fold's trained model to disk as JSON

        # Permutation importance per fold
        pi = permutation_importance(bst, X_tr, Y_tr, n_repeats=5, random_state=123, scoring="neg_mean_squared_error")  # shuffle each feature 5 times and measure the rise in MSE
    ...
    plot_loss_curves(eval_histories, plots_dir, tag=tag)
    ...
    bst_best = XGBRegressor(objective='reg:squarederror', seed=42)
    bst_best.load_model(os.path.join(model_dir, f"best_model_cv{best_fold}.json"))  # restore best fold's trained model from disk
    bst_best.get_booster().feature_names = list(feature_columns)                    # attach feature names so SHAP plot labels use column names
    logger.info(f"  Computing SHAP beeswarm on best CV fold (fold {best_fold}, RMSE={rmse_arr[best_fold]:.4f}) training data ...")
    shap_vals_best = compute_shap_values(bst_best, X_tr_best, feature_columns)      # exact Shapley values via TreeExplainer on training subjects
    ...
    final_model.fit(X_train_all, Y_train_all, verbose=False)                        # train on all training subjects without a validation split
    final_model.get_booster().feature_names = list(feature_columns)                 # attach column names so plots use names not f0/f1/...
    final_model_path = os.path.join(model_dir, "final_model.json")
    final_model.save_model(final_model_path)                                        # persist final model as JSON for later loading and inference
    logger.info(f"Final model saved to {final_model_path}")

    # Evaluate final model on held-out test set
    test_preds = final_model.predict(X_test)                                         # predict ages for the 200 held-out test subjects
    ...
    plot_xgb_importance(final_model, plots_dir, tag=tag)
    plot_permutation_importance(final_model, X_test, Y_test, feature_columns, plots_dir, tag=tag)  # importance measured on unseen test data
...
def main():
    ...
    # Hyperparameter search ranges (multiple values trigger grid search)
    parser.add_argument('--n_estimators', type=int, nargs='+', default=[100], help="Values for n_estimators in grid search")
    parser.add_argument('--max_depth', type=int, nargs='+', default=[6], help="Values for max_depth in grid search")
    parser.add_argument('--learning_rate', type=float, nargs='+', default=[0.3], help="Values for learning_rate in grid search")
    parser.add_argument('--gamma', type=float, nargs='+', default=[0], help="Values for gamma in grid search")
    ...
    args = parser.parse_args()                                                     # parse command-line arguments into the args namespace
    ...
    DEFAULT_PARAMS = {                                                              #Run 1 - Default hyperparameters
        'learning_rate': 0.3,
        'gamma': 0,
        'max_depth': 6,
        'n_estimators': 100,
    ...
    grid_params = {                                                                 #Grid Search over candidate hyperparameter ranges
        'learning_rate': args.learning_rate, # default [0.3]
        'n_estimators': args.n_estimators, # default [100]
        'gamma': args.gamma, # default [0]
        'max_depth': args.max_depth, # default [6]
    }
    n_combinations = (len(args.learning_rate) * len(args.n_estimators) * len(args.gamma) * len(args.max_depth)) # total number of hyperparameter combinations to evaluate
    logger.info("")
    ...
    logger.info(f"{'R^2':<8}  {default_metrics['R2']:>10.4f}  {best_metrics['R2']:>10.4f}")
    logger.info("")
    logger.info(f"Final model (best params) stored at: {args.output_dir}/best/models/final_model.json")
    ...
    logger.info(f"{'R^2':<8}  {best_metrics['R2']:>10.4f}")
    logger.info("")
    logger.info(f"Final model (best params) stored at: {args.output_dir}/best/models/final_model.json")

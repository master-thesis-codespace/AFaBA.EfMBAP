...
#    )
    ...
    handles = [
        Line2D([0], [0], marker="o", color="w", markerfacecolor=_BLUE_DOT, markersize=7, label="SAF-HC (2nd corr.)"),
        Line2D([0], [0], color=_BLUE_LINE, linestyle="--", linewidth=1.5, label="SAF-HC linear fit"), Line2D([0], [0], marker="o", color="w", markerfacecolor=_RED_DOT, markersize=7, label="SAF-Patient (2nd corr.)"),
        Line2D([0], [0], color=_RED_LINE, linestyle="--", linewidth=1.5, label="SAF-Patient linear fit"),
    ]
...
def plot_histogram(df_hc: pd.DataFrame, df_pt: pd.DataFrame, out_path: str, title: str) -> None:
    ...
    ax.hist(cor_hc, bins=bins, weights=w_hc, color=_BLUE_DOT, alpha=0.55, edgecolor="white", linewidth=0.5, label="SAF-HC (2nd corr.)", zorder=2)
    ax.hist(cor_pt, bins=bins, weights=w_pt, color=_RED_DOT, alpha=0.55, edgecolor="white", linewidth=0.5, label="SAF-Patient (2nd corr.)", zorder=3)
    ...
    handles = [
        plt.Rectangle((0, 0), 1, 1, color=_BLUE_DOT, alpha=0.55, label="SAF-HC (2nd corr.)"),
        Line2D([0], [0], color=_BLUE_LINE, linestyle="--", linewidth=1.5, label=f"SAF-HC mean ({np.mean(cor_hc):.1f})"),
        plt.Rectangle((0, 0), 1, 1, color=_RED_DOT, alpha=0.55, label="SAF-Patient (2nd corr.)"),
        Line2D([0], [0], color=_RED_LINE, linestyle="--", linewidth=1.5, label=f"SAF-Patient mean ({np.mean(cor_pt):.1f})"),
    ]
...
def plot_violin(df_hc: pd.DataFrame, df_pt: pd.DataFrame, healthy_name: str, patient_name: str, out_path: str, title: str) -> None:
    ...
    ax.set_xticks([0, 1])
    ax.set_xticklabels(["Healthy Cohort", "SAF-Patient (2nd corr.)"], fontsize=9)

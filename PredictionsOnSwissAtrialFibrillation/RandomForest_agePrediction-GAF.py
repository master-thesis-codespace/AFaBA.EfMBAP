...
def preproc_data(file_path: str, feature_columns: Optional[List[str]] = None, return_ID: bool = False):
    ...
    if feature_columns is None:
        if df_data.shape[1] > 3:
            feature_columns = df_data.columns.to_list()[3:]                             #default features: Sex + all morphometrics (index 3 onwards) - matches RandomForest_age_model_modelling-GAF.py preproc_data()
        else:
...
def prediction_brain_age_MS(file_path, best_model_pkl_file, HC_prediction_json_file, output_csv_filename):
    ...
    #Setup Model
    bst = joblib.load(best_model_pkl_file)                                              #load the pretrained Random Forest model - joblib.load() returns the fully fitted estimator, no reconstruction needed
...
def build_argparser():                                                                  #run per model fold
    ...
    p.add_argument("--best_model_pkl_file",help="Pickled (.pkl) file of the best trained Random Forest model (GAF variant) in cross-validation", required=True)

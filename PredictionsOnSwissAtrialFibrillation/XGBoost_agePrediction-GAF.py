...
def preproc_data(file_path: str, feature_columns: Optional[List[str]] = None, return_ID: bool = False):
    ...
    if feature_columns is None:
        if df_data.shape[1] > 3:
            feature_columns = df_data.columns.to_list()[3:]                             #default features: Sex + all morphometrics (index 3 onwards) - matches XGBoost_age_model_modelling-GAF.py preproc_data()
        else:
...
def prediction_brain_age_MS(file_path, best_model_json_file, HC_prediction_json_file, output_csv_filename, param_dict):
    ...
    #Setup Model
    bst = XGBRegressor(objective="reg:squarederror", seed=42, **param_dict)             #initialize the model with defined hyperparameters
    bst.load_model(best_model_json_file)                                                #load the pretrained model; load_model() restores the full trained booster, so param_dict has no effect from here on.
    if getattr(bst, "best_iteration", None) is not None:                               #best_model_cv{i}.json (trained with eval_set + early stopping) has this set and survives save/load;
        bst.set_params(n_estimators=bst.best_iteration)                                 #final_model.json (trained on all data, no eval_set) does not - guard so both artifacts load without crashing.
...
def build_argparser():                                                                  #run per model fold
    ...
    p.add_argument("--best_model_json_file",help="JSON file of the best trained XGBoost model (GAF variant) in cross-validation", required=True)
...
def main():
    ...
    param_dict = {"max_depth": args.max_depth, "learning_rate": args.learning_rate}

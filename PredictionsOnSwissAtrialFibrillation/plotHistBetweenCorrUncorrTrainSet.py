...
def main() -> None:
    ap = argparse.ArgumentParser(description="Brain-age-gap histogram: uncorrected vs corrected (training set / OOF cross-validation)")
    ap.add_argument("--output_dir", default=".", help="Model output folder containing best/results/cv_final_predictions.csv")
    args = ap.parse_args()
    dir_label = os.path.basename(os.path.abspath(args.output_dir))

    print(f"Loading training-set (OOF cross-validation) predictions: {args.output_dir}")
    df = load_predictions(args.output_dir)

    print("Generating histogram ...")
    title = f"Brain-Age Gap - Differences on the Training Set (OOF Cross-Validation) - {dir_label}"
    out_path = os.path.join(args.output_dir, f"{dir_label}_hist_uncorVsCor_train.png")

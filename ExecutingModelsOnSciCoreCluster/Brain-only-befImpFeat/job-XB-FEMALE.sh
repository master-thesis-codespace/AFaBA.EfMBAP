...
# ---------------------------------------------------------------------------
#SBATCH --job-name=BOTH_XB_beImpFeat_FEMALE-GS8         # A name for your job. Shows up in `squeue`
...
#SBATCH --time=12:00:00                  # Maximum wall-clock time (HH:MM:SS). When this is hit,
...
TARGET="BOTH_XB_beImpFeat_FEMALE-GS8"
srun python XGBoost_age_model_modelling.py \
  --input BOTH_befImpFeat-FEMALE.csv \
  --output_dir "$TARGET" \
  --n_estimators 4100 4200 4300 4600 \

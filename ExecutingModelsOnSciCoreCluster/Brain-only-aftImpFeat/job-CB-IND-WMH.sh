...
# ---------------------------------------------------------------------------
#SBATCH --job-name=BRAIN_CB_aftImpFeat_IND_WMH-GS9         # A name for your job. Shows up in `squeue`
...
#SBATCH --qos=1day                      # Quality Of Service = the time/priority class.
...
#SBATCH --mem=16GB                     # Total RAM (CPU memory, NOT GPU memory) for the job.
                                        #   If your job uses more than this, SLURM kills it.
                                        #   256GB is generous; lower it if you don't need that much.
#SBATCH --time=20:00:00                  # Maximum wall-clock time (HH:MM:SS). When this is hit,
...
TARGET="BRAIN_CB_aftImpFeat_IND_WMH-GS9"
srun python CatBoost_age_model_modelling.py \
  --input BRAIN_INDIVIDUALaftImpFeat_WMH-CB.csv \
  --output_dir "$TARGET" \
  --iterations 8000 10000 12000 \
